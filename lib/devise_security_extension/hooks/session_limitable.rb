...
Warden::Manager.after_set_user :only => :fetch do |record, warden, options|
  ...
  if record.respond_to?(:unique_session_id) && warden.authenticated?(scope) && options[:store] != false && (!record.respond_to?(:skip_session_limitable?) || !record.skip_session_limitable?)
